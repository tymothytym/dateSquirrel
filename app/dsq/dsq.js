...
if (!rnDsq.dsq) {
	rnDsq.dsq = function() {
		...
		const defaults = { // dsq defaults
			    ...
			    classPrefix: 'dsq-', // prefix JS-added classes - if changed scss variabl "$dsq-prefix" must be updated
			    ...
			    parseDelay: 0 // the delay in ms before dateSquirrel will parse, format and rewrite a user given date
			},
		...
		window.dsq = class dsq {
			...
			makeLists() {
				...
				if (this.hasMonth) {
					for (let m = 0; m < 12; m++) {
						this.lists.months.insertAdjacentHTML('beforeend', '<li id="' + this.uid + '_m_' + m + '" data-month="' + m + '" tabindex="0" role="option">' + monthList[m] + '</li>');
					}
				} else {
				...
				this.mthClickFn = {
					handleEvent: function (e) {
						if (!e.target.classList.contains(that.options.classPrefix + 'disabled')) {
							...
							if (that.options.day) {
								that.o.value = monthList[that.selectedMonth] + ' ' + that.selectedYear;
				...
				if (!this.wrapper.classList.contains('dsq-touch') && this.options.parse) {
					this.addEvt(this.o, 'keyup', this.keyupFn, false);
				}
			...
			addDays() {
				let numberOfDays = dsq.daysInMonth(this.selectedYear, this.selectedMonth),
					...
					firstOfMonth;
				this.reminder.innerHTML = '<a class="' + this.options.classPrefix + 'reminder-month">' + monthList[this.selectedMonth] + '</a>&nbsp;<a class="' + this.options.classPrefix + 'reminder-year">' + this.selectedYear + '</a>';
			...
			finishUp(makeDate) {
				...
				if (makeDate === undefined) {
					...
					if (dsq.isValidDate(dsq.parseDateString(makeDate))) {
						this.setDate = dsq.parseDateString(makeDate);
			...
			static parseDateString(str) {
				...
				switch (true) {
					...
					case str.indexOf(':') > 0:
						...
						break;
					default:
						// uh oh, unrecognised delimiter
						//console.error('Uh oh, unrecognised delimiter');
						return false;
				}
				...
				if (parts.length === 3) { // e.g day, month, year
					...
					//console.log('parts[1] * 1: ', parts[1] * 1);
					if (isNaN(parts[1] * 1)) {
						monthNum = monthList.indexOf(parts[1].substring(0,3).charAt(0).toUpperCase() + parts[1].substring(0,3).slice(1));
						//console.log('monthNum: ', monthNum);
					} else {
						monthNum = parts[1]-1;
					}
					if (isNaN(parts[0] * 1)) {
						dayNum = parts[0].replace(/\D/g,'');
					} else {
						dayNum = parts[0];
					}
					...
					//console.log('parts[2].length: ', parts[2].length);
					if (parts[2]) {
						if (parts[2].length === 4) {
							yearNum = parts[2];
						} else if (parts[2].length === 3) {
							yearNum = parts[2].replace(/\D/g,'');
						} else {
							if (parts[2] * 1 > new Date().getFullYear().toString().slice(-2)) {
								yearNum = `19${parts[2]}`;
							} else {
								yearNum = `20${parts[2]}`;
							}
